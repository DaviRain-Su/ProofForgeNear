#!/usr/bin/env bash
# Engineering local-node gate for bounded one-field AccountId JSON object input.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() { echo "near-local-json-account-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-account-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonAccountInput.wasm"
echo "near-local-json-account-input: building fixture" >&2
lake exe pf -- build --target near --out build/near NearJsonAccountInput
[[ -f "$wasm" ]] || die "missing $wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-account-input.XXXXXX")"
sandbox_pid=""
cleanup() {
...
"$python" runtime-tests/near/json_account_input.py
echo "near-local-json-account-input: ok"
