...
skip() { echo "near-local-token-storage: skip: $*" >&2; exit 0; }
die() { echo "near-local-token-storage: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearTokenStorage.wasm"
lake exe pf -- build --target near --out build/near NearTokenStorage
[[ -f "$wasm" ]] || die "missing $wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-token-storage.XXXXXX")"
sandbox_pid=""
cleanup() {
...
"$python" runtime-tests/near/token_storage.py
echo "near-local-token-storage: ok"
