#!/usr/bin/env bash
# Engineering local-node gate for the guest-owned Wasm invocation arena.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-memory: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-memory: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearMemory.wasm"
echo "near-local-memory: building NearMemory.wasm" >&2
lake exe pf -- build --target near --out build/near NearMemory
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-memory.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-memory: near-sandbox init --home $home" >&2
...
echo "near-local-memory: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-memory: RPC ready; running allocator scenes" >&2
"$python" runtime-tests/near/memory.py
echo "near-local-memory: ok"
