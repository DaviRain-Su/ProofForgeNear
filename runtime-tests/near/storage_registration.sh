#!/usr/bin/env bash
# Engineering local-node gate for closed caller-only registration economics.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() { echo "near-local-storage-registration: skip: $*" >&2; exit 0; }
die() { echo "near-local-storage-registration: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearStorageRegistration.wasm"
lake exe pf -- build --target near --out build/near NearStorageRegistration
[[ -f "$wasm" ]] || die "missing $wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-storage-registration.XXXXXX")"
sandbox_pid=""
cleanup() {
...
"$python" runtime-tests/near/storage_registration.py
echo "near-local-storage-registration: ok"
