...
- `ProofForge/Wasm/Near/Sdk/**` must not import same-target Emit / Assemble / Registry.
"""
...
SDK_FORBIDDEN_IMPORT = re.compile(
    r"^\s*import\s+ProofForge\.Wasm(?:\.(?P<target>Near))?(?:\.[A-Za-z0-9_]+)*\."
...
def main() -> int:
    ...
    for path in iter_sdk_files():
        ...
        for match in SDK_FORBIDDEN_IMPORT.finditer(text):
            ...
            elif f"ProofForge/Wasm/{target}/" in posix or posix.startswith(
                f"ProofForge/Wasm/{target}/"
            ):
                failures.append(
                    f"{rel}:{line_number(text, match.start())}: "
                    f"SDK modules must not import {match.group('kind')}: "
