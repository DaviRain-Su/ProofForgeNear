#!/usr/bin/env bash
# Engineering local-node gate for the closed internal fungible ledger foundation.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-ledger: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-ledger: FAIL: $*" >&2
  exit 1
}
...
for _ in $(seq 1 90); do
  ...
  if ! kill -0 "$sandbox_pid" 2>/dev/null; then
    ...
    die "near-sandbox exited before RPC became ready"
  fi
  sleep 1
done
[[ "$ready" == 1 ]] || { cat "$workdir/node.log" >&2 || true; die "near-sandbox RPC not ready"; }
