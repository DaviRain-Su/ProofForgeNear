#!/usr/bin/env bash
# Engineering local-node gate for exact bounded Option<StorageBalance> JSON output.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() { echo "near-local-storage-balance-output: skip: $*" >&2; exit 0; }
die() { echo "near-local-storage-balance-output: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearStorageBalanceOutput.wasm"
echo "near-local-storage-balance-output: building fixture" >&2
lake exe pf -- build --target near --out build/near NearStorageBalanceOutput
[[ -f "$wasm" ]] || die "missing $wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-storage-balance-output.XXXXXX")"
sandbox_pid=""
cleanup() {
...
"$python" runtime-tests/near/storage_balance_output.py
echo "near-local-storage-balance-output: ok"
