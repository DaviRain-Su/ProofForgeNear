#!/usr/bin/env bash
# Engineering local-node gate for Identity IterableMap/IterableSet storage.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-iterable: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-iterable: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearIterable.wasm"
echo "near-local-iterable: building NearIterable.wasm" >&2
lake exe pf -- build --target near --out build/near NearIterable
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-iterable.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-iterable: near-sandbox init --home $home" >&2
...
echo "near-local-iterable: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-iterable: RPC ready; running Iterable scenes" >&2
"$python" runtime-tests/near/iterable.py
echo "near-local-iterable: ok"
