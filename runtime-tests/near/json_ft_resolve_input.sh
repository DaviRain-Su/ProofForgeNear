...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-ft-resolve-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-ft-resolve-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonFtResolveInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonFtResolveInput
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-ft-resolve-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_ft_resolve_input.py
echo "near-local-json-ft-resolve-input: ok"
