#!/usr/bin/env bash
# Engineering local-node gate for bounded direct-write NEAR Vector storage.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-vector: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-vector: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearVector.wasm"
echo "near-local-vector: building NearVector.wasm" >&2
lake exe pf -- build --target near --out build/near NearVector
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-vector.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-vector: near-sandbox init --home $home" >&2
...
echo "near-local-vector: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-vector: RPC ready; running Vector scenes" >&2
"$python" runtime-tests/near/vector.py
echo "near-local-vector: ok"
