#!/usr/bin/env bash
# Engineering local-node gate for NearLazy. Missing near-sandbox -> skip (exit 0).
#
# Deploys ProofForge NearLazy.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-lazy: skip: $*" >&2
  exit 0
}
die() {
  echo "near-local-lazy: FAIL: $*" >&2
  exit 1
}

sandbox="${PF_TOOL_ROOT:-$HOME/.cache/proofforge-tool-root}/linux-x64/near-sandbox"
if [[ ! -x "$sandbox" ]]; then
...
if ! "$python" -c "import base58" >/dev/null 2>&1; then
  skip "python base58 unavailable (see runtime-tests/near/requirements.txt)"
fi

wasm="${PWD}/build/near/NearLazy.wasm"
echo "near-local-lazy: building NearLazy.wasm" >&2
lake exe pf -- build --target near --out build/near NearLazy
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-lazy.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-lazy: near-sandbox init --home $home" >&2
...
echo "near-local-lazy: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-lazy: RPC ready; running lazy cell scenes" >&2
"$python" runtime-tests/near/lazy.py
echo "near-local-lazy: ok"