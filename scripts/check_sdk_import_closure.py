#!/usr/bin/env python3
"""Assert ProofForge.Wasm.Near.Sdk transitive imports never reach Emit/Assemble/Registry.
...
def module_to_path(mod: str) -> Path | None:
    ...
    if parts[0] != "ProofForge":
        return None
    # ProofForge.Wasm.Near.Sdk → ProofForge/Wasm/Near/Sdk.lean
...
def is_forbidden(mod: str) -> bool:
    if not mod.startswith("ProofForge.Wasm."):
        return False
    rest = mod[len("ProofForge.Wasm.") :]
