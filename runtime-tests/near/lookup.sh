#!/usr/bin/env bash
# Engineering local-node gate for direct Identity NEAR LookupMap/LookupSet storage.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-lookup: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-lookup: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearLookup.wasm"
echo "near-local-lookup: building NearLookup.wasm" >&2
lake exe pf -- build --target near --out build/near NearLookup
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-lookup.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-lookup: near-sandbox init --home $home" >&2
...
echo "near-local-lookup: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-lookup: RPC ready; running lookup scenes" >&2
"$python" runtime-tests/near/lookup.py
echo "near-local-lookup: ok"
