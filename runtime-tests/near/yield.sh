#!/usr/bin/env bash
# Engineering local-node gate for NearYield. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearYield.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-yield: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-yield: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearYield.wasm"
echo "near-local-yield: building NearYield.wasm" >&2
lake exe pf -- build --target near --out build/near NearYield
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-yield.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-yield: near-sandbox init --home $home" >&2
...
echo "near-local-yield: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-yield: RPC ready; running account/u128/height/time scenes" >&2
"$python" runtime-tests/near/yield.py
echo "near-local-yield: ok"
