...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-ft-transfer-call-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-ft-transfer-call-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonFtTransferCallInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonFtTransferCallInput
[[ -f "$wasm" ]] || die "missing $wasm"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-ft-transfer-call-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_ft_transfer_call_input.py
echo "near-local-json-ft-transfer-call-input: ok"
