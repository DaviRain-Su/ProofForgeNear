...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-boolean-mutation: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-boolean-mutation: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonBooleanMutation.wasm"
lake exe pf -- build --target near --out build/near NearJsonBooleanMutation
[[ -f "$wasm" ]] || die "missing $wasm"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-boolean-mutation.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_boolean_mutation.py
echo "near-local-json-boolean-mutation: ok"
