#!/usr/bin/env bash
# Engineering local-node gate for exact bounded StorageBalanceBounds JSON output.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() { echo "near-local-storage-balance-bounds-output: skip: $*" >&2; exit 0; }
die() { echo "near-local-storage-balance-bounds-output: FAIL: $*" >&2; exit 1; }
...
[[ -n "$version" ]] || skip "near-sandbox present but not runnable on this host"
...
wasm="${PWD}/build/near/NearStorageBalanceBoundsOutput.wasm"
echo "near-local-storage-balance-bounds-output: building fixture" >&2
lake exe pf -- build --target near --out build/near NearStorageBalanceBoundsOutput
[[ -f "$wasm" ]] || die "missing $wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-storage-balance-bounds-output.XXXXXX")"
sandbox_pid=""
cleanup() {
  if [[ -n "${sandbox_pid:-}" ]] && kill -0 "$sandbox_pid" 2>/dev/null; then
    kill "$sandbox_pid" 2>/dev/null || true; wait "$sandbox_pid" 2>/dev/null || true
  fi
  rm -rf "$workdir"
}
trap cleanup EXIT
...
for _ in $(seq 1 90); do
  if curl -sf -X POST "$rpc" -H 'content-type: application/json' \
      -d '{"jsonrpc":"2.0","id":"1","method":"status","params":[]}' >/dev/null 2>&1; then ready=1; break; fi
  if ! kill -0 "$sandbox_pid" 2>/dev/null; then tail -80 "$workdir/node.log" >&2 || true; die "near-sandbox exited early"; fi
  sleep 0.5
done
[[ "$ready" -eq 1 ]] || { tail -80 "$workdir/node.log" >&2 || true; die "RPC not ready"; }

export PF_NEAR_RPC="$rpc" PF_NEAR_HOME="$home" PF_NEAR_WASM="$wasm"
export PYTHONPATH="${PWD}/runtime-tests/near${PYTHONPATH:+:$PYTHONPATH}"
"$python" runtime-tests/near/storage_balance_bounds_output.py
echo "near-local-storage-balance-bounds-output: ok"
