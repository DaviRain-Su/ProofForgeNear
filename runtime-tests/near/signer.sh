#!/usr/bin/env bash
# Engineering local-node gate for NearSigner. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearSigner.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-signer: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-signer: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearSigner.wasm"
echo "near-local-signer: building NearSigner.wasm" >&2
lake exe pf -- build --target near --out build/near NearSigner
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-signer.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-signer: near-sandbox init --home $home" >&2
...
echo "near-local-signer: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-signer: RPC ready; running signer id/pk scenes" >&2
"$python" runtime-tests/near/signer.py
echo "near-local-signer: ok"
