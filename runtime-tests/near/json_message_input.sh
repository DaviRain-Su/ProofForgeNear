...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-message-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-message-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonMessageInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonMessageInput
[[ -f "$wasm" ]] || die "missing $wasm"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-message-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_message_input.py
echo "near-local-json-message-input: ok"
