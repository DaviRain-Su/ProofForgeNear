#!/usr/bin/env bash
# Engineering local-node gate for canonical one-field quoted-u128 amount JSON input.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() { echo "near-local-json-amount-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-amount-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonAmountInput.wasm"
echo "near-local-json-amount-input: building fixture" >&2
lake exe pf -- build --target near --out build/near NearJsonAmountInput
[[ -f "$wasm" ]] || die "missing $wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-amount-input.XXXXXX")"
sandbox_pid=""
cleanup() {
...
"$python" runtime-tests/near/json_amount_input.py
echo "near-local-json-amount-input: ok"
