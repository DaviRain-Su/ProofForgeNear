...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-storage-deposit-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-storage-deposit-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonStorageDepositInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonStorageDepositInput
[[ -f "$wasm" ]] || die "missing $wasm"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-storage-deposit-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_storage_deposit_input.py
echo "near-local-json-storage-deposit-input: ok"
