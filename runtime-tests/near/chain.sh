#!/usr/bin/env bash
# Engineering local-node gate for NearChain. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearChain.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-chain: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-chain: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearChain.wasm"
echo "near-local-chain: building NearChain.wasm" >&2
lake exe pf -- build --target near --out build/near NearChain
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-chain.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-chain: near-sandbox init --home $home" >&2
...
echo "near-local-chain: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-chain: RPC ready; running epoch/gas/locked/seed scenes" >&2
"$python" runtime-tests/near/chain.py
echo "near-local-chain: ok"
