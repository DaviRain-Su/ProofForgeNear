#!/usr/bin/env bash
# Engineering local-node gate for NearTreeMap. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearTreeMap.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-treemap: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-treemap: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearTreeMap.wasm"
echo "near-local-treemap: building NearTreeMap.wasm" >&2
lake exe pf -- build --target near --out build/near NearTreeMap
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-treemap.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-treemap: near-sandbox init --home $home" >&2
...
echo "near-local-treemap: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-treemap: RPC ready; running account/u128/height/time scenes" >&2
"$python" runtime-tests/near/treemap.py
echo "near-local-treemap: ok"
