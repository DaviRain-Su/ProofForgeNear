...
skip() { echo "near-local-ft-event: skip: $*" >&2; exit 0; }
die() { echo "near-local-ft-event: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearFungibleTokenEvent.wasm"
echo "near-local-ft-event: building NearFungibleTokenEvent.wasm" >&2
lake exe pf -- build --target near --out build/near NearFungibleTokenEvent
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-ft-event.XXXXXX")"
sandbox_pid=""
cleanup() {
...
"$python" runtime-tests/near/ft_event.py
echo "near-local-ft-event: ok"
