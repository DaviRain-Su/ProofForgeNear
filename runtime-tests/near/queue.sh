#!/usr/bin/env bash
# Engineering local-node gate for the ProofForge bounded persistent NEAR Queue.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-queue: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-queue: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearQueue.wasm"
echo "near-local-queue: building NearQueue.wasm" >&2
lake exe pf -- build --target near --out build/near NearQueue
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-queue.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-queue: near-sandbox init --home $home" >&2
...
echo "near-local-queue: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-queue: RPC ready; running Queue scenes" >&2
"$python" runtime-tests/near/queue.py
echo "near-local-queue: ok"
