#!/usr/bin/env bash
# Engineering local-node gate for NearCtx. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearCtx.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-context: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-context: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearCtx.wasm"
echo "near-local-context: building NearCtx.wasm" >&2
lake exe pf -- build --target near --out build/near NearCtx
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-context.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-context: near-sandbox init --home $home" >&2
...
echo "near-local-context: starting node rpc=127.0.0.1:${rpc_port}" >&2
