...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-ft-transfer-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-ft-transfer-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonFtTransferInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonFtTransferInput
[[ -f "$wasm" ]] || die "missing $wasm"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-ft-transfer-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_ft_transfer_input.py
echo "near-local-json-ft-transfer-input: ok"
