#!/usr/bin/env bash
# Engineering local-node gate for callback-result count and out-of-range reads.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-promise-result: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-promise-result: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearPromiseResult.wasm"
echo "near-local-promise-result: building NearPromiseResult.wasm" >&2
lake exe pf -- build --target near --out build/near NearPromiseResult
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-promise-result.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-promise-result: near-sandbox init --home $home" >&2
...
echo "near-local-promise-result: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-promise-result: RPC ready; running callback-result substrate scenes" >&2
"$python" runtime-tests/near/promise_result.py
echo "near-local-promise-result: ok"
