...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-ft-on-transfer-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-ft-on-transfer-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonFtOnTransferInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonFtOnTransferInput
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-ft-on-transfer-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_ft_on_transfer_input.py
echo "near-local-json-ft-on-transfer-input: ok"
