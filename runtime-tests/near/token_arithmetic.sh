#!/usr/bin/env bash
# Engineering local-node gate for checked little-endian u128 token arithmetic.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() { echo "near-local-token-arithmetic: skip: $*" >&2; exit 0; }
die() { echo "near-local-token-arithmetic: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearTokenArithmetic.wasm"
lake exe pf -- build --target near --out build/near NearTokenArithmetic
[[ -f "$wasm" ]] || die "missing $wasm"
"$python" -I -S -c 'import sys; raise SystemExit(0 if open(sys.argv[1], "rb").read(4) == b"\x00asm" else 1)' "$wasm" || die "$wasm is not wasm"

workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-token-arithmetic.XXXXXX")"
sandbox_pid=""
cleanup() {
...
c.setdefault("network", {})["addr"] = "127.0.0.1:${net_port}"
c.setdefault("network", {})["boot_nodes"] = ""
...
for _ in $(seq 1 90); do
  ...
  if ! kill -0 "$sandbox_pid" 2>/dev/null; then
    tail -80 "$workdir/node.log" >&2 || true; die "near-sandbox exited early"
  fi
  sleep 0.5
done
[[ "$ready" -eq 1 ]] || { tail -80 "$workdir/node.log" >&2 || true; die "near-sandbox RPC not ready"; }

export PF_NEAR_RPC="$rpc" PF_NEAR_HOME="$home" PF_NEAR_WASM="$wasm"
export PYTHONPATH="${PWD}/runtime-tests/near${PYTHONPATH:+:$PYTHONPATH}"
"$python" runtime-tests/near/token_arithmetic.py
echo "near-local-token-arithmetic: ok"
