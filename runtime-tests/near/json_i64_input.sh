#!/usr/bin/env bash
# Engineering local-node gate for json_i64_input. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearJsonI64Input.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-i64: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-i64: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearJsonI64Input.wasm"
echo "near-local-i64: building NearJsonI64Input.wasm" >&2
lake exe pf -- build --target near --out build/near NearJsonI64Input
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json_i64_input.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-i64: near-sandbox init --home $home" >&2
...
echo "near-local-i64: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-i64: RPC ready; running account/u128/height/time scenes" >&2
"$python" runtime-tests/near/json_i64_input.py
echo "near-local-i64: ok"
