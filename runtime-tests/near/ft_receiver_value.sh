...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-ft-receiver-value: skip: $*" >&2; exit 0; }
die() { echo "near-local-ft-receiver-value: FAIL: $*" >&2; exit 1; }
...
if ! "$python" -c "from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey; import base58" >/dev/null 2>&1; then skip "python cryptography+base58 unavailable"; fi
receiver_wasm="${PWD}/build/near/NearFtReceiverValue.wasm"
promise_wasm="${PWD}/build/near/NearPromise.wasm"
lake exe pf -- build --target near --out build/near NearFtReceiverValue NearPromise
[[ -f "$receiver_wasm" && -f "$promise_wasm" ]] || die "missing receiver/Promise artifacts"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-ft-receiver-value.XXXXXX")"; sandbox_pid=""
...
export PF_NEAR_RPC="$rpc" PF_NEAR_HOME="$home" PF_NEAR_RECEIVER_WASM="$receiver_wasm" PF_NEAR_PROMISE_WASM="$promise_wasm" PYTHONPATH="${PWD}/runtime-tests/near${PYTHONPATH:+:$PYTHONPATH}"
"$python" runtime-tests/near/ft_receiver_value.py
echo "near-local-ft-receiver-value: ok"
