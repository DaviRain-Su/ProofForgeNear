#!/usr/bin/env bash
# Engineering local-node gate for NearCrypto. Missing near-sandbox → skip (exit 0).
# Deploys ProofForge NearCrypto.wasm only; never runs cargo/near-sdk.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-crypto: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-crypto: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearCrypto.wasm"
echo "near-local-crypto: building NearCrypto.wasm" >&2
lake exe pf -- build --target near --out build/near NearCrypto
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-crypto.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-crypto: near-sandbox init --home $home" >&2
...
echo "near-local-crypto: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-crypto: RPC ready; running hash/ecrecover/ed25519 scenes" >&2
"$python" runtime-tests/near/crypto.py
echo "near-local-crypto: ok"
