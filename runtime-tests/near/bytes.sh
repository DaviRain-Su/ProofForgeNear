#!/usr/bin/env bash
# Engineering local-node gate for canonical Borsh bounded bytes/string input.
set -euo pipefail
cd "$(dirname "$0")/../.."

skip() {
  echo "near-local-bytes: skip: $*" >&2
  exit 0
}

die() {
  echo "near-local-bytes: FAIL: $*" >&2
  exit 1
}
...
wasm="${PWD}/build/near/NearBytes.wasm"
echo "near-local-bytes: building NearBytes.wasm" >&2
lake exe pf -- build --target near --out build/near NearBytes
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-bytes.XXXXXX")"
sandbox_pid=""
cleanup() {
...
home="$workdir/home"
mkdir -p "$home"
echo "near-local-bytes: near-sandbox init --home $home" >&2
...
echo "near-local-bytes: starting node rpc=127.0.0.1:${rpc_port}" >&2
...
echo "near-local-bytes: RPC ready; running Borsh/UTF-8 scenes" >&2
"$python" runtime-tests/near/bytes.py
echo "near-local-bytes: ok"
