...
set -euo pipefail
cd "$(dirname "$0")/../.."
skip() { echo "near-local-json-storage-withdraw-input: skip: $*" >&2; exit 0; }
die() { echo "near-local-json-storage-withdraw-input: FAIL: $*" >&2; exit 1; }
...
wasm="${PWD}/build/near/NearJsonStorageWithdrawInput.wasm"
lake exe pf -- build --target near --out build/near NearJsonStorageWithdrawInput
[[ -f "$wasm" ]] || die "missing $wasm"
workdir="$(mktemp -d "${TMPDIR:-/tmp}/near-json-storage-withdraw-input.XXXXXX")"; sandbox_pid=""
...
"$python" runtime-tests/near/json_storage_withdraw_input.py
echo "near-local-json-storage-withdraw-input: ok"
