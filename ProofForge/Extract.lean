...
def decodeBody (env : Environment) (e : Expr) (preserveLocals : Bool := false)
  ...
  -- Storage effects need the same lexical boundary: a value captured before a write or a
  -- promise/storage effect must not be substituted into that later effect and re-read
  -- after the mutation.
  ...
  let retainLets := hasStructuredState || hasSequencedNearEffects
...
def decodeMutating (env : Environment) (e : Expr) (stateType? : Option Name := none) :
  ...
  if Ops.hasCheckedArith ops || writesOptionLeaf 8 ops || hasIte ops ||
      Ops.hasNearEffect ops || Ops.hasLangOp ops ||
...
/-- Wire-compatible scalar widths shared by the Wasm targets (no target module dependency). -/
...
/-- Expand a top-level bounded result into its fixed scalar frame before the target chooses an
output wire format. This is source projection only: length and padding remain target
owned. v1 admits wide scalars and one-level static tuple/record elements; dynamic
...
def extractMethod (env : Environment) (kind : Core.IR.MethodKind) (n : Name) :
  ...
  let rec capturedStateArg (fuel : Nat) (v : Ops.Val) : Nat :=
    ...
    | fuel' + 1 =>
      ...
      match v with
      | .field (.arg i) _ => i
      | .field base _ | .bitNot base => capturedStateArg fuel' base
  ...
  let retTypes :=
    ...
    | .scalar type => #[type]
    | _ => #[]
  let annotations := Attr.nearEntryAnnotations env n
  return {
...
private partial def valEscapedArg (limit : Nat) : Ops.Val → Option Nat
  ...
  | .field b _ | .bitNot b => valEscapedArg limit b
  | .bitAnd l r | .bitOr l r | .bitXor l r | .shiftL l r | .shiftR l r
  | .addU64 l r | .subU64 l r | .mulU64 l r | .divU64 l r | .modU64 l r =>
